...
# Date: 2010-07-01
#
# File Description: eSearch/eFetch NCBI genbank HIV protein sequences database
#
# Date: 2016-09-15
#
...
my $baseDirectory = "$viroblastDB/protein/hiv";
...
my $fastaFile = "$baseDirectory/hiv_prot.fas";
my $newFastaFile = "$newDirectory/hiv_prot.fas";
my $tempFastaFile = "$newDirectory/hiv_prot_temp.fas";
my $newAccFile = "$newDirectory/hiv_prot.acc";
my $tempAccFile = "$newDirectory/hiv_prot_temp.acc";
my $logFile = "$newDirectory/hiv_prot_makeblastdb.log";
my $dbLogFile = "$newDirectory/hiv_prot_acc.log";
my $accErrorFile = "$newDirectory/hiv_prot_acc.err";
my $downloadfastaErrorFile = "$newDirectory/hiv_prot_download.err";
my $cleanfastaErrorFile = "$newDirectory/hiv_prot_clean.err";
my (@accs, @accnums);
...
if(@accs) {
	...
	if (@new_accs) {
		...
		open ACC, ">>", "$newAccFile" || die "Couldn't open $newAccFile: $!\n";
		open OUT, ">>", "$newFastaFile" || die "Couldn't open $newFastaFile: $!\n";
	...
	print LOG "$datestring:\tInserted $new_acc_source NCBI genbank accessions to ViroBLAST HIV protein sequence database.";
	
	if ($new_acc_source) {
		my $rv1 = system("makeblastdb", '-in', $newFastaFile, '-dbtype', 'prot', '-logfile', $logFile);
		if ($rv1 == 0) {
			...
			@newfiles = grep {$_ ne "$newDirectory/hiv_prot.fas"} @newfiles;
			@newfiles = grep {$_ ne "$newDirectory/hiv_prot_temp.fas"} @newfiles;
			for my $newfile (@newfiles) {
				copy ($newfile, $baseDirectory) or die "copy failed $newfile: $!\n";
			}
			my @rmfiles = glob("$newDirectory/hiv_prot.fas.*");
...
my $statFile = "$statDir/downloadHIV_prot.log";

...
# Date: 2006-01-12
#
# File Description: eSearch/eFetch NCBI genbank viral sequence database
#
# Date: 2016-09-15
#
...
my $baseDirectory = "$viroblastDB/nucleotide/viral";
...
my $fastaFile = "$baseDirectory/gbvrl.fas";
my $newFastaFile = "$newDirectory/gbvrl.fas";
my $tempFastaFile = "$newDirectory/gbvrl_temp.fas";
my $newAccFile = "$newDirectory/gbvrl.acc";
my $tempAccFile = "$newDirectory/gbvrl_temp.acc";
my $logFile = "$newDirectory/gbvrl_makeblastdb.log";
my $dbLogFile = "$newDirectory/gbvrl_acc.log";
my $accErrorFile = "$newDirectory/gbvrl_acc.err";
my $downloadfastaErrorFile = "$newDirectory/gbvrl_download.err";
my $cleanfastaErrorFile = "$newDirectory/gbvrl_clean.err";
my (@accs, @accnums);
...
if(@accs) {
	...
	print LOG "$datestring:\tInserted $new_acc_source NCBI genbank accessions to ViroBLAST viral sequence database.";
	
	if ($new_acc_source) {
		my $rv1 = system("makeblastdb", '-in', $newFastaFile, '-dbtype', 'nucl', '-logfile', $logFile, '-max_file_sz', '4GB');
		if ($rv1 == 0) {
			...
			@newfiles = grep {$_ ne "$newDirectory/gbvrl.fas"} @newfiles;
			@newfiles = grep {$_ ne "$newDirectory/gbvrl_temp.fas"} @newfiles;
			for my $newfile (@newfiles) {
				copy ($newfile, $baseDirectory) or die "copy failed $newfile: $!\n";
			}
			my @rmfiles = glob("$newDirectory/gbvrl.fas.*");
...
my $statFile = "$statDir/downloadVrl_nucl.log";
